...
import com.berbin.util.ConnectionUtil;
	...
	public List<Trains> searchTrain(LocalDate DepartureDate, String source, String destination) {
		...
		try {
			...
			String findTrain = "select train_id,train_name,train_class,train_number,train_source,train_destination,"
					+ "train_departure_time,train_arraival_time,total_seat,ticket_price from trains "
					+ "where to_char(train_departure_time,'dd-mm-yyyy')=? and train_source=? and train_destination=?";

			Trains trainModel;
			trainsearchList = new ArrayList<Trains>();
